...
const serializeSessionState = async (session) => {
  // Defensive: detect duplicate questionOrder entries and collapse them for serialized state
  const seenIds = new Set();
  const duplicatesInOrder = [];
  const normalizedQuestionOrder = [];
  for (const entry of session.questionOrder || []) {
    const qid = String(entry.question);
    if (seenIds.has(qid)) {
      duplicatesInOrder.push(qid);
      continue;
    }
    seenIds.add(qid);
    normalizedQuestionOrder.push(entry);
  }
  if (duplicatesInOrder.length) {
    logExamIntegrity('question-duplicates-detected', {
      reason: 'serialize-session-duplicates',
      duplicates: Array.from(new Set(duplicatesInOrder)),
      sessionId: String(session._id),
    });
  }
  const ids = normalizedQuestionOrder.map((entry) => entry.question);
  ...
  return {
    ...
    questions: normalizedQuestionOrder.map((entry) =>
...
const autoExpireIfNeeded = async (session) => {
  ...
  const expireResult = await ExamSession.updateOne(
    { _id: session._id, user: session.user, status: 'active' },
    {
      $set: {
        status: 'expired',
        submittedAt: expiredNow,
        isSubmitting: false,
  ...
  // If modifiedCount is 0, another concurrent request already transitioned this session
  // (or it's no longer active) - that's fine, we just reflect the current state below
  // instead of racing on a non-atomic session.save().
  session.status = 'expired';
  session.submittedAt = expiredNow;
  session.isSubmitting = false;
  session.lastActivityAt = expiredNow;
  if (expireResult.modifiedCount) {
...
const createExamSession = async ({ user, mode, examType, sectionSubject, strictNavigation = false }) => {
  ...
  // Defensive dedupe: ensure no duplicate question IDs before persisting
  const seenQ = new Set();
  const duplicatesFound = [];
  const dedupedSessionQuestions = [];
  for (const q of shuffledSessionQuestions) {
    ...
    seenQ.add(id);
    dedupedSessionQuestions.push(q);
  }
  if (duplicatesFound.length) {
    logExamIntegrity('question-duplicates-detected', {
      reason: 'create-session-duplicates',
      duplicates: Array.from(new Set(duplicatesFound)),
      examType: resolvedExam,
      mode,
    });
  }
  // Use deduped sequence for persistence
  const finalSessionQuestions = dedupedSessionQuestions;
  const expectedTotal = Number(questionCount || 0);
  const actualQuestionCount = finalSessionQuestions.length;
  ...
  const hardValidation = validateBlueprintHard({
    ...
    selectedQuestions: finalSessionQuestions,
    examType: resolvedExam,
  });
  ...
  const session = await ExamSession.create({
    ...
    version: 0,
    lastAnsweredIndex: -1,
    questionOrder: finalSessionQuestions.map((question) => ({
...
const submitAnswer = async ({
  ...
  // NOTE: This bookkeeping write is intentionally an atomic findOneAndUpdate rather than
  // `session.save()`. The schema has `optimisticConcurrency: true`, so a plain `.save()`
  // throws an uncaught VersionError whenever two requests race on this document - which
  // happens routinely, since the frontend's single-flight lock is per-question, not
  // per-session (a save for question N can still be in flight when a save for question
  // N+1 starts). That race previously caused a perfectly valid answer submission to be
  // rejected with a generic 400 purely because of a rate-counter conflict unrelated to
  // the answer itself. Using $inc/$set here makes this write commutative and race-free,
  // consistent with the atomic updateOne already used for the answer write further below.
  ...
  const bookkeepingUpdate = await ExamSession.findOneAndUpdate(
    { _id: sessionId, user: userId },
    {
      $set: {
        lastActivityAt: now,
        answerRateWindowStartedAt: answerRateWindow.nextStartedAt,
        answerRateCount: answerRateWindow.nextCount,
      },
      $inc: {
  ...
  if (!bookkeepingUpdate) {
    const error = new Error('Exam session not found');
    error.statusCode = 404;
    throw error;
  }

  // Keep the in-memory `session` document (used throughout the rest of this function)
  // in sync with the fields we just wrote atomically, without touching its version key.
  session.lastActivityAt = bookkeepingUpdate.lastActivityAt;
  session.answerRateWindowStartedAt = bookkeepingUpdate.answerRateWindowStartedAt;
  session.answerRateCount = bookkeepingUpdate.answerRateCount;
  session.answerAttemptCount = bookkeepingUpdate.answerAttemptCount;
  ...
  if (processedIntent) {
    ...
    const duplicateReplayNonce = generateOpaqueToken();
    const duplicateReplayNow = new Date();
    await ExamSession.updateOne(
      { _id: sessionId, user: userId },
      { $set: { nextRequestNonce: duplicateReplayNonce, lastActivityAt: duplicateReplayNow } }
    );
    session.nextRequestNonce = duplicateReplayNonce;
    session.lastActivityAt = duplicateReplayNow;
...
const submitExamSession = async ({ userId, sessionId }) => {
  ...
  // Atomic for the same reason as the equivalent write in submitAnswer: this bookkeeping
  // update runs before the isSubmitting lock below, so a double-click on Submit (or a
  // retried request) can race two of these calls on the same document. Using $set here
  // instead of session.save() avoids an uncaught VersionError knocking out the second
  // request before it ever reaches the (already-safe) idempotent/lock-conflict handling.
  const submitBookkeeping = await ExamSession.findOneAndUpdate(
    { _id: sessionId, user: userId },
    {
      $set: {
        submitRateWindowStartedAt: submitRateWindow.nextStartedAt,
        submitRateCount: submitRateWindow.nextCount,
        lastActivityAt: now,
      },
    },
    { new: true }
  ...
  if (!submitBookkeeping) {
    const error = new Error('Exam session not found');
    error.statusCode = 404;
    throw error;
  }

  session.submitRateWindowStartedAt = submitBookkeeping.submitRateWindowStartedAt;
  session.submitRateCount = submitBookkeeping.submitRateCount;
  session.lastActivityAt = submitBookkeeping.lastActivityAt;
