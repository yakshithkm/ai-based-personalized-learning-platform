...
describe('ExamSimulationPage integrity failure handling', () => {
  ...
  it('dedupes duplicate questions from backend on restore', async () => {
    // Create a session with duplicate questions to simulate a backend issue
    const base = buildSession({ selectedAnswerIndex: 0, version: 3, ledgerSeq: 1 });
    // Duplicate first question intentionally
    const dupQuestion = base.questions[0];
    const newQuestions = [dupQuestion, dupQuestion, ...base.questions.slice(1)];
    const dupSession = { ...base, questions: newQuestions, questionCount: newQuestions.length };

    api.post.mockResolvedValue({ data: dupSession });

    // Start simulation (which uses createExamSession -> POST)
    await startSimulation();

    // Palette buttons reflect unique questions only
    const paletteBtns = screen.getAllByRole('button').filter((btn) => btn.querySelector('.palette-number'));
    const uniqueCount = new Set(newQuestions.map((q) => q._id)).size;
    expect(paletteBtns.length).toBe(uniqueCount);
  });
